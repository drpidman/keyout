...
if (isset($_GET["action"]) && $_GET["action"] == "new") {
    ...
    if (!$sala) {
        header("Location:" . baseRedirect("reservas?nova-reserva=new&erro=empty_sala"));
        return;
    }
    ...
    if (!$stmt->rowCount()) {
        isset($_GET["mb_redirect"]) ?
            header("Location: " . baseRedirect("mobileview?erro=user_not_found"))
            :
            header("Location:" . baseRedirect("reservas?nova-reserva=new&sala=") . $sala . "&erro=user_not_found");
        return;
    }
    ...
    try {
        ...
        if (isset($_GET["mb_redirect"])) {
            header("Location:" . $_GET["mb_redirect"]);
            return;
        }

        header("Location:" . baseRedirect("reservas"));
        ...
        header("Location:" . baseRedirect("reservas?nova-reserva=new&sala=") . $sala . "&erro=create_error");
...
if (isset($_GET["action"]) && $_GET["action"] == "confirm") {
    ...
    if ($select_sala == "Selecione uma sala") {
        header("Location:" . baseRedirect("reservas?&erro=empty_sala&from_page=change_status&user_id=") . $user_id);
    }
    ...
    try {
        ...
        header("Location:" . baseRedirect("reservas?action=confirmar&erro=failed_to_update"));
        return;
    }
}


if (isset($_GET["action"]) && $_GET["action"] == "confirm_sala") {
    $select_sala = $_POST["select_sala"];
    $redirect = $_GET["redirect"];
...
if (isset($_GET["action"]) && $_GET["action"] == "get_reserved") {
    ...
    if (!$stmt->rowCount()) {
        header("Location:" . baseRedirect("reservas?erro=user_not_found&from_page=search"));
        return;
    }

    $usuario = $stmt->fetch(PDO::FETCH_OBJ);
    header("Location:" . baseRedirect("reservas?action=confirmar&method=change_status&user_id=") . $usuario->idusuario);
}
