    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <a type="button" class="btn-close" href="<?= baseRedirect("reservas") ?>"></a>
            </div>
            <?php if (isset($_GET["method"]) && $_GET["method"] == "search"): ?>
                <form class="modal-body" action="<?= baseRedirect("reservas/actions.php?action=get_reserved") ?>"
                    method="POST">
                    <div class="mb-3">
                        <label for="input_nregistro" class="form-label">Numero de registro</label>
                        <input type="text" class="form-control" id="input_nregistro" name="nregistro"
                            aria-describedby="nregistro_help">
                        <div id="nregistro_help" class="form-text">
                            Digite seu numero de registro para buscar as salas reservadas
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-primary" type="submit">Buscar salas</button>
                    </div>
                </form>
            <?php endif; ?>
            <?php if (isset($_GET["method"]) && $_GET["method"] == "change_status"):
                ...
                <?php if ($query_reservas->rowCount()): ?>
                    <form class="modal-body"
                        action="<?= baseRedirect("reservas/actions.php?action=confirm&redirect=") . baseRedirect("reservas") ?>"
                        method="POST">
                ...
                <?php else: ?>
                    ...
                    <div class="modal-footer">
                        <a href="<?= baseRedirect("reservas") ?>" class="btn btn-primary">Fechar</a>
                    </div>
                <?php endif ?>
            <?php endif; ?>
