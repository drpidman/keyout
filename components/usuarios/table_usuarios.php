      ...
      <?php while ($usuario = $stmt_usuarios->fetch(PDO::FETCH_ASSOC)) : ?>
        <div class="d-flex justify-content-between align-items-center p-3 border rounded">
          <div class="p-2 d-flex flex-column align-items-start">
            <h2>
              <i class="ri-user-line"></i> <?= $usuario["nome"] ?>
            </h2>
            <p class="text-body-tertiary">
              <i class="ri-pass-pending-line"></i> <?= $usuario["nregistro"] ?>
            </p>
          </div>
          <div>
            <a href="<?= baseRedirect("usuarios?novo-usuario=edit") ?>&id=<?= $usuario["idusuario"] ?>" class="btn fs-4"><i class="ri-pencil-line"></i></a>
            <a href="<?= baseRedirect("usuarios?action=delete-user") ?>&id=<?= $usuario["idusuario"] ?>" class="btn fs-4"><i class="ri-delete-bin-2-line"></i></a>
          </div>
        </div>
      <?php endwhile ?>
    </div>
