...
<section class="container mt-3">
  <div class="mb-3">
    <a href="?nova-sala=new" class="btn btn-primary">
    ...
    <div class="d-flex flex-column gap-3 mt-4">
      <?php while ($usuario = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
        <div
          ...
          <div>
            <a href="<?= baseRedirect("salas?nova-sala=edit") ?>&id=<?= $usuario['idsala'] ?>" class="btn fs-4 rounded"><i
                class="ri-pencil-line"></i></a>
            <a href="<?= baseRedirect("salas?action=delete-sala") ?>&id=<?= $usuario['idsala'] ?>"
              class="btn fs-4 rounded"><i class="ri-delete-bin-2-line"></i></a>
          </div>
        </div>
      <?php endwhile ?>
    </div>
